...
/* 	select [distinct] <colunas>
  ...
  select pedidos.id, nome from pedidos inner join clientes on pedidos.cliente_id = clientes.id;
    ...
    -- 3
    select pedidos.id, pedidos.data, clientes.nome from pedidos inner join clientes on pedidos.cliente_id = clientes.id
    ...
    -- 5
	select pedidos.id, nome from pedidos left join clientes on pedidos.cliente_id = clientes.id;
	...
	select sum(pedidos.valor), clientes.nome from pedidos inner join clientes on pedidos.cliente_id = clientes.id group by clientes.nome;
 ...
 select * from itens_pedido;
    ...
    select clientes.nome, pedidos.id, pedidos.data from clientes
    inner join pedidos on pedidos.cliente_id = clientes.id
...
group by nome;
    ...
    select nome, pedidos.valor from pedidos inner join clientes on pedidos.cliente_id = clientes.id group by nome order by valor desc limit 1;
    
    select * from pedidos;
    
    -- 15 Qual a média de pizzas por pedido e quantos pedidos foram feitos?
    select count(distinct pedido_id) as total_pedidos, avg(quantidade) as media from itens_pedido;
    ...
    select max(pedidos.valor), min(pedidos.valor), clientes.nome from pedidos inner join clientes  on pedidos.cliente_id = clientes.id where nome = 'Laura Madureira' group by nome ;
	...
	select min(pedidos.valor), clientes.nome from pedidos inner join clientes  on pedidos.cliente_id = clientes.id group by nome order by valor asc limit 1;

	-- 20 Liste todos os clientes, mesmo que não tenham feito pedidos, com seus respectivos pedidos (se houver).
	select clientes.nome, pedidos.id as 'id do pedido' from clientes left join pedidos on clientes.id = pedidos.cliente_id;
    
    -- 21 Liste todos os clientes com o valor total de seus pedidos (mesmo que não tenham feito pedidos).
	select clientes.nome, sum(pedidos.valor) as valor_total, count(pedidos.id) as total_pedidos from pedidos right join clientes on clientes.id = pedidos.cliente_id group by clientes.id, clientes.nome;

	-- 22 Liste os 3 clientes que mais gastou, exibir nome do cliente e o valor gasto.
    select clientes.nome, pedidos.valor from pedidos inner join clientes on pedidos.cliente_id = clientes.id group by nome order by valor desc limit 3;
